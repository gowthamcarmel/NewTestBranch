 ...
 * 2.00       30 Apr 2014     dicasiano        add removal of WT lines before approval
 * 3.00		  20 May 2014	  asinsin		   add validation of Notify Vendor on related Purchase Order before approving the Bill
 *
 */
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						if (b3PPPO == 'T')
						{
							nlapiLogExecution('DEBUG', stLoggerTitle, 'Next Check Notify Vendor of Related PO on Vendor Bill = ' + stVB);
							// Verify if this still satisfies this check **SCRIPT USE: Notify Vendor Check
							var arrNotifyVendorCheckResult = nlapiSearchRecord('transaction', 'customsearch_notify_vendor_check', filter);
							if(arrNotifyVendorCheckResult)
							{
								nlapiLogExecution('DEBUG', stLoggerTitle, 'Proceed to Next Record. Vendor Bill = ' + stVB + ' did not satisfy Notify Vendor Check');
								continue;
							}
						}
						else
						{
							...
							else
							{
								...
								{   
									...
									var cnt = billRec.getLineItemCount('item');
									for (var i=1; i<=cnt; i++){
										var isWT = billRec.getLineItemValue('item', 'custcol_4601_witaxline', i);
										nlapiLogExecution('DEBUG', stLoggerTitle, 'isWt = ' + isWT );
										if(isWT == 'T')
										{
											billRec.removeLineItem('item', i);
											nlapiLogExecution('DEBUG', stLoggerTitle, 'remove line item = ' + i);
										}
									}
									
									
									billRec.setFieldValue('approvalstatus', '2');
									var recId = nlapiSubmitRecord(billRec);
									//nlapiSubmitField('vendorbill', stVB, 'approvalstatus', '2');
									nlapiLogExecution('DEBUG', stLoggerTitle, 'Approved Vendor Bill = ' + recId);
