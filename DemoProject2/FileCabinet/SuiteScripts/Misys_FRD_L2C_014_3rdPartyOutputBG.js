...
$Revision: 6  0v2 MC - added 3pp fields jan16$
*/
...
function NI(sr)
{
	...
	this.txt3ppClient = Utils.srValue(sr,'custcol_3pp_client');
...
*/
		...
		{
			var merge = nlapiMergeRecord(this.emailTemplateID,'contact',nf.contacts[0].id,'purchaseorder',nf.poID); // use first contact
			//var merge = nlapiMergeRecord(this.emailTemplateID,'contact',nf.contactID,'purchaseorder',nf.poID);
...
Doc.prototype.createBFO = function(runDateStr,nf)
{
	...
	for(var i=0;i<nf.items.length;i++)
	{
		...
		html+='<tr><td class="item">Event Narrative:</td><td>'+nlapiEscapeXML(ni.eventNarr)+'</td><td>'+
