...
function scheduled(type) {
	try
	{
		...
		//if (!__is.inArray(['create','edit'], type) ) return __log.end('Ignoring type: ' + type, true);	
		//if (!__is.inArray(['workflow','userevent','userinterface','suitelet','scheduled'], exec) ) return __log.end('Ignoring execution context:' + exec, true);
		
		var searchResults = nlapiSearchRecord('itemreceipt', 'customsearch1846');
		if (searchResults != null) {
			for ( var int = 0; int < searchResults.length; int++) {
				var result = searchResults[int];
				var receipt = result.getId();
				
				var recItemRcpt = nlapiLoadRecord('itemreceipt', receipt);
