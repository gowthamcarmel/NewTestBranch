...
function workflowAction_enterRejectReason()
{
    ...
    {    	
    	...
    	nlapiSetRedirectURL('SUITELET','customscript_mys_upd_so_rej_rsn','customdeploy_mys_upd_so_rej_rsn', null, arrParams);
...
function showRejectReasonPage(request, response, form, stTranId, stTranType)
{
	...
	if (stTranType.indexOf('Authorisation') !== -1) {
		form.addField('custpage_reject_reason_details', 'text', 'Rejection Reason Details').setMandatory(true);
	} else {
		form.addField('custpage_reject_reason_details', 'text', 'Rejection Reason Details').setMandatory(true);
		form.addField('custpage_reject_reason', 'select', 'Rejection Reason', '430').setMandatory(true); // UAT
	}
...
function updateRejectReason(request, response, form, stTranId, stTranType)
{
	...
	if (stTranType.indexOf('Authorisation') !== -1) {
		...
		nlapiSubmitField(stTranType, stTranId, ['custbody_mys_so_rej_reason', 'custbody_mys_so_rej_det', 'custbody_mys_approval_stat'], 
				[stRejectReason, stRejectReasonDetails, '3']);
	}
