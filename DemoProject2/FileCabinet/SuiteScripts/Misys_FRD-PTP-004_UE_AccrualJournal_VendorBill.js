...
function scheduled_AccrualJournalonVendorBill()
{
	try
	{
		...
		//by gowthaman
		var data = '';
		var rowno = new Array(),csvFolderId ='424998', errorExists = false;
		// prod folder id = 424998 ,sb =386362
		  
		rowno[0] = 'Vendor Bill internalid';
		  rowno[1] = 'Reason for Failure';
		  data += rowno + '\n';
		...
		else		
		{
			...
			var arrSearch = nlapiSearchRecord('vendorbill', null, 
					 [
					  ...
					  ,(new nlobjSearchFilter('isinactive', 'subsidiary', 'is', 'F' ) )
                                        //  ,(new nlobjSearchFilter('isinactive', 'department', 'is', 'F' ) )
                                       //,(new nlobjSearchFilter('internalid', null, 'noneof', 3439908,3250968 ) )
                                      // ,(new nlobjSearchFilter('internalid', null, 'anyof', 3439908 ) )
					 ]
					,[
			...
			for ( var ii in arrSearch)
			{
			  ...
			  {
				  //creates data for writing into csv
				  //added by gowthaman
				  
				  rowno[0] = arrSearch[ii].getValue('internalid');
				  rowno[1] = e.getDetails();
				  data += rowno + '\n';
		...
		//creates csv file and saves into file cabinet
		//added by gowthaman
		try
		{ 
			if(errorExists)
				{
		var newFile = nlapiCreateFile('inconsistent_vendor_bill_'+new Date()+'_.csv', 'CSV', data); 
	...
	{			
		...
		for ( var iii in arrJELines)
		{
			...
			recNewJournal.selectNewLineItem('line');
                       ...
                       nlapiLogExecution( 'ERROR', 'expenseaccount',rowVB.getValue('expenseaccount','item'));
                       nlapiLogExecution( 'ERROR', 'custitem_misysaccrualaccount',rowVB.getValue('custitem_misysaccrualaccount','item'));
