...
var context = nlapiGetContext();
	...
	var stJECustomForm = context.getSetting('SCRIPT', 'custscript_slarr_je_custom_form');
    ...
    if (isEmpty(stCustomerTransSummarySearch) || isEmpty(stCustomerTransDetailSearch) || isEmpty(stEmailAlert) || isEmpty(stJECustomForm))
    {	
...
{	
	...
	params['custscript_arr_je_custom_form'] = stJECustomForm;
...
{    
    ...
    {    	
		...
		var stJECustomForm = context.getSetting('SCRIPT', 'custscript_arr_je_custom_form');
    	nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Customer Transactions Summary Search = ' + stCustomerTransSummarySearch
				...
				+ ' | JE Custom Form = ' + stJECustomForm);
    	//stJECustomForm = 111
    	if (isEmpty(stCustomerTransSummarySearch) || isEmpty(stCustomerTransDetailSearch) || isEmpty(stEmailAlert) || isEmpty(bSendEmail) || isEmpty(stBatchId) || isEmpty(stCurrentUser) || isEmpty(stSrcTranPeriod) || isEmpty(stNettingDate) || isEmpty (stAcctsReceivable) || isEmpty(stJECustomForm))
    	{
    		...
    		for (var z = 0; z < arrICBatchesQueue.length; z++)
            {
    	    	...
    	    	if (arrIntercompanyEntityMapping != null)
    	        {
    	        	...
    	        	for (var i = 0; i < arrIntercompanyEntityMapping.length; i++)
    	            {
						...
						var stCustomerRepresentsSubsidiary = nlapiLookupField('customer', stCustomer, 'representingsubsidiary');
    	        		...
    	        		var arrCustomerTransactionsSummary = getCustomerTransactionsSummary(stSubsidiary, stSrcTranPeriodEnd, stCustomerTransSummarySearch, stCustomer, stAcctsReceivable);
    	        		if (arrCustomerTransactionsSummary != null)
    	        	    {
    	        			...
    	        			for (var j = 0; j < arrCustomerTransactionsSummary.length; j++)
    	                    {
    	            			...
    	            			else if (flTotalForeignAmount < 0)
    	            			{
									...
									var stJournalEntry = createJournalEntry(stCustomer, stSubsidiary, stCustomerRepresentsSubsidiary, stCurrency, stNettingDate, stAcctsReceivable, stCostCenter, stProduct, stRegion, stNettingBankAcct, flTotalForeignAmount, stJECustomForm);
...
{	
	try
	{
		...
		var arrCustomerTransactionsDetail = getTransactionsToApply(stSubsidiary, stCurrency, stSrcTranPeriodEnd, stCustomer, stAcctsReceivable, stCustomerTransDetailSearch);
		if (arrCustomerTransactionsDetail != null)
	    {
	        ...
	        {	
				...
				if (stTranType == 'invoice' || stTranType == 'journalentry' || stTranType == 'intercompanyjournalentry')
				{
					...
					nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Debit count ' + intCustomerPaymenApplyCount);
					for (var j = 1; j <= intCustomerPaymenApplyCount; j++)
					{
						...
						nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Debit Line = Tran ID ' + stDoc == stTranInternalId);
						nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Debit Line = ' + stDoc);
						nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Tran ID = ' + stTranInternalId);
				...
				if (stTranType == 'creditmemo'|| stTranType == 'journalentry' || stTranType == 'intercompanyjournalentry')
				{
					...
					nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Credit Line Count ' + intCustomerPaymenCreditCount);
					for (var k = 1; k <= intCustomerPaymenCreditCount; k++)
					{
						// If the doc field is equal to the Internal ID, the script sets the apply field to T
						var stDoc = recCustomerPayment.getLineItemValue('credit', 'doc', k);
						nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Credit Line = Tran ID ' + stDoc == stTranInternalId);
						nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Credit Line = ' + stDoc);
						nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Tran ID = ' + stTranInternalId);
						
						if (stDoc == stTranInternalId)
						{
							recCustomerPayment.selectLineItem('credit', k);
							recCustomerPayment.setCurrentLineItemValue('credit', 'apply', 'T');
							recCustomerPayment.commitLineItem('credit');
							nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Credit Line ' + k + ' is applied to the Customer Payment');
							bHasApplyLine = true;
						}
					}
				}
		...
		if (bHasApplyLine)
		{
			...
			//pshah 16-10-14 --> if customer payment balanced the account field should not be updated otherwise will throw an error if it tries to.
			if (stCustomerPayment!=0 && stCustomer!= null){
...
function createJournalEntry(stVendor, stSubsidiary, stVendorRepresentsSubsidiary, stCurrency, stNettingDate, stAcctsPayable, stCostCenter, stProduct, stRegion, stNettingBankAcct, flTotalForeignAmount, stJECustomForm)
{
	...
	try
	{
		...
		var recJournalEntry = nlapiCreateRecord('journalentry', {recordmode:'dynamic'});
		...
		recJournalEntry.setCurrentLineItemValue('line', 'account', stAcctsPayable); // Netting Bank Account from Intercompany Entity Mapping search result
		...
		recJournalEntry.setCurrentLineItemValue('line', 'account', stNettingBankAcct); // Accounts Payable Account defined when running the routine
...
{	
	try
	{
		...
		nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Trans to apply? ' + (arrCustomerTransactionsDetail != null));
		if (arrCustomerTransactionsDetail != null)
	    {
			// For each transaction Internal ID from the search result
			var intCustomerTransactionsDetailLength = arrCustomerTransactionsDetail.length;
	        ...
	        {	
				...
				nlapiLogExecution('DEBUG', LOGGER_TITLE, 'transToApply = ' + intCustomerRefundApplyCount);
 ...
 * @returns
 */
function getCustomerTransactionsSummary(stSubsidiary, stSrcTranPeriodEnd, stCustomerTransSummarySearch, stCustomer,stAcctsReceivable)
{
    var arrFilters = [new nlobjSearchFilter('subsidiary', null, 'anyof', stSubsidiary),
					  ...
					  new nlobjSearchFilter('account', null, 'anyof', stAcctsReceivable)];
 ...
 * @returns
 */
function getTransactionsToApply(stSubsidiary, stCurrency, stSrcTranPeriodEnd, stCustomer, stAcctsReceivable, stCustomerTransDetailSearch)
{
    var arrFilters = [new nlobjSearchFilter('subsidiary', null, 'anyof', stSubsidiary),
                      new nlobjSearchFilter('trandate', null, 'onorbefore', stSrcTranPeriodEnd),
                      new nlobjSearchFilter('entity', null, 'anyof', stCustomer),
					  ...
					  new nlobjSearchFilter('currency', null, 'anyof', stCurrency),
                      new nlobjSearchFilter('account', null, 'anyof', stAcctsReceivable)]; //18/08/14 dicasiano@netsuite.com
