...
var LOGGER_TITLE = 'Mark Notify Vendor on RLF Invoice Payment';

var SCHED_SCRIPT_ID = 'customscript_mark_notify_vedor_on_rlf_ip';

var USAGE_LIMIT_THRESHOLD = 100;

/**
 * When the customer sale invoice becomes fully paid, the corresponding Purchase Order lines are marked as Notify Vendor. 
 * Note that Purchase Orders are not automatically created once a Sales Invoice is generated. Lines are created in Pending 3PP Events record. 
 * A user will select these lines at a required interval and manually trigger the creation of the Purchase Order. 
 * This process means that a customer payment may have been received before the Purchase Order is actually.
 * 
 * @author Aurel Shenne Sinsin
 * @version 1.0
 */
function scheduled_markNotifyVendorOnRLFnvPayment()
{
	try
    {  
    	...
    	var stEmailRecipient = context.getSetting('SCRIPT', 'custscript_mnvrp_email_recipient');
    	var stEmailAlert = context.getSetting('SCRIPT', 'custscript_mnvrp_email_alert');        
        var stSavedSearch = context.getSetting('SCRIPT', 'custscript_mnvrp_saved_search');
    	...
    	if (arrPOSearchResult != null)
    	{
    		...
    		for (var i = 0; i < intPOSearchResultCount; i++)
            {
    	        ...
    	        if (intRemainingUsage < USAGE_LIMIT_THRESHOLD)
    	        {
    	    		...
    	    		params['custscript_mnvrp_email_recipient'] = stEmailRecipient;
    	    		params['custscript_mnvrp_email_alert'] =  stEmailAlert;
    	    		params['custscript_mnvrp_saved_search'] = stSavedSearch;
