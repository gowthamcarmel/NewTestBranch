...
function beforeSubmit_ReverseAccrualJE(type)
{
	try
	{
		...
		//o   Determine the “Accrual Journal Entry Number”  Field on item receipt record as the Original JE
		var stAccrualJE = recItemRcpt.getFieldValue('custbody_accrualje_no');
		
		//o   IF “Accrual Journal Entry Number” field is Empty  
		//o   Then    Exit  
...
function beforeLoad_ShowDeleteBtnItemRecpt(type, form, request)
{
	try
	{
		...
		//hide the delete button if posting period closed or PO status = closed
		...
		var recPO = nlapiGetFieldValue('createdfrom');
		
		if(recPO!="" && recPO!=null){
			var searchFilter = [new nlobjSearchFilter('internalid',null,'anyof',recPO)];
			var searchRecord = nlapiSearchRecord(null,'customsearch_closed_pos',searchFilter);
			if(searchRecord!=null && searchRecord!="") return true; 	
		}
