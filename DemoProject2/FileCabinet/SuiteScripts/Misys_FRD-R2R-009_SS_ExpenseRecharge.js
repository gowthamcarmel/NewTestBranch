...
function _create_ICRecharge( tranType, subsId, MAPVALUES ) {
	...
	arrColumns.push( new nlobjSearchColumn('fxamount') );
	...
	for (var ii in arrSearchResults)
	{
		...
		var jobData = _JOB_DATA[dataProjectIC];
		if (!jobData) {
			jobData = nlapiLookupField('job', dataProjectIC, ['recordtype','custentity_excludeicrecharge']);
			_JOB_DATA[dataProjectIC] = jobData;
		}
		
		var stProjectICType = jobData && jobData['recordtype'] ? jobData['recordtype'] : false;
		if(!stProjectICType || stProjectICType != 'job') continue;  // skip if Project IC is not a Project
		var excludeFromRecharges = jobData['custentity_excludeicrecharge'];
		...
		var stAmount	= __fn.parseFloat( resultRow.getValue('fxamount') );
		dataIC['custrecord_icc_rate_fcy'] = stAmount;
		dataIC['custrecord_icc_amount_fcy'] = stAmount;
		
		__log.writev('*** Data', [dataIC]);
		var itemData = _ITEM_DATA[dataIC['custrecord_icc_item']];
		if( !itemData ) {
			itemData = nlapiLookupField('item', dataIC['custrecord_icc_item'], ['custitem_category','custitem_subcat1','custitem_subcat2']);
			_ITEM_DATA[dataIC['custrecord_icc_item']] = itemData;
		}
		...
		if ( excludeFromRecharges!='T' && stProjectIC && stCostCtr && stRegion && stProduct)
		{
			var uniqKey = [ dataIC['custrecord_icc_source_subsidiary'], 
			                ...
			                resultRow.getId(), stProjectIC, stCostCtr, stProduct, stRegion ].join('||');
		...
		__log.writev('/// Creating recovery point ///');
		
		var state = nlapiSetRecoveryPoint(); 
		__log.writev('Recovery point ***', [state]);		
		if (! __usage.hasRemaining('80%') )		
		{
	        var state = nlapiYieldScript();
	        var reason = 'Reason: ' + state.reason + ' Info: ' + state.information + ' Size: ' + state.size;
	        __log.writev('** Yield Script', [reason]);
	        
	        if (state.status == 'FAILURE')
	        {        	
	            return __error.report('!!!! Failed to yield script. Exiting... ' + reason);
	        }
	        else if (state.status == 'RESUME')
	        {
	        	__log.writev('////////// Resuming script because of ..', [reason]);
	        	__log.start({
		       		 'logtitle'  : 'ExpenseRecharges-SS'
		       		,'company' 	 : 'Misys'
		       		,'scriptname': 'Misys_FRD-RDR-009_SL_ExpenseRecharges.js'
...
function _RescheduleScript()
{
    ...
    //jkbautista : 20141009 - Checking the SSDEV guide for SuiteScript, getDeploymentId() Returns the deploymentId for the current script deployment (ie., the currently executing script)
    //jkbautista : 20141009 - This proves that removing unnecessary deployments within the script will not harm the execution, as it is only re-scheduling ITSELF( cx.getDeploymentId())
...
var _MAP_BILL = {
	 'custrecord_icc_date' 				: 'trandate'
	,'custrecord_icc_currency' 			: 'currency' // currency of the record
	 ,'custrecord_icc_period' 			: false //'postingperiod'
