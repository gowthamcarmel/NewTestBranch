...
var LOGGER_TITLE = 'Mark Notify Vendor on ILF Invoice Payment';

var SCHED_SCRIPT_ID = 'customscript_mark_notify_vedor_on_ilf_ip';

var USAGE_LIMIT_THRESHOLD = 100;

/**
 * When the customer sales invoice linked to the 3PP billing milestone is fully paid, the corresponding Purchase Order lines are marked as Notify Vendor. 
 ...
 * This process means that a customer payment may have been received before the Purchase Order is actually created and marked for notification
 * 
 * @author Aurel Shenne Sinsin
 * @version 1.0
 */
function scheduled_markNotifyVendorOnILFnvPayment()
{
	try
    {  
    	...
    	var stEmailRecipient = context.getSetting('SCRIPT', 'custscript_mnvip_email_recipient');
    	var stEmailAlert = context.getSetting('SCRIPT', 'custscript_mnvip_email_alert');        
        var stSavedSearch = context.getSetting('SCRIPT', 'custscript_mnvip_saved_search');
    	...
    	if (arrPOSearchResult != null)
    	{
    		...
    		for (var i = 0; i < intPOSearchResultCount; i++)
            {
    	        ...
    	        if (intRemainingUsage < USAGE_LIMIT_THRESHOLD)
    	        {
    	    		...
    	    		params['custscript_mnvip_email_recipient'] = stEmailRecipient;
    	    		params['custscript_mnvip_email_alert'] =  stEmailAlert;
    	    		params['custscript_mnvip_saved_search'] = stSavedSearch;
    	        ...
    	        var stVendorMilestone = arrPOSearchResult[i].getValue('custcol_3pp_vendor_milestone');
    	        ...
    	        nlapiLogExecution('DEBUG', LOGGER_TITLE, 'PO Search Result: Purchase Order = '  + stPO
    	        		...
    	        		+ '\n <br /> Vendor Milestone = ' + stVendorMilestone
    	        ...
    	        // Search if the Transaction and Milestone is used on the Customer Billing Schedule
    	        var stCustomerBillingSched = searchCustomerBillingSchedule(st3PPSourceTransaction, st3PPSourceTransactionType, stVendorMilestone);
    	        nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Customer Billing Schedule for 3PP Source Transaction (Invoice/Sales Order) and Milestone. ID = '  + stCustomerBillingSched);
    	   
    	        if (!isEmpty(stCustomerBillingSched))
    	        {
    	        	if (bLoadPO)
        	        {
            	        ...
            	        if (arrTranid.length > 0)
            	        {
                	        ...
                	        {	
                	        	...
                	        	for (var j = 0; j < arrUniquePending3PPEventsResult; j++)
                		        {
                		        	...
                		        	if (!isSOInvFound)
                		        	{
                		        		nlapiSubmitField(stType, st3PPSourceTran, 'custbody_notification_processed', 'T');
                		        		nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Successfully updated Notification Process to true. ID = ' + st3PPSourceTran
                		        				+ '\n <br /> Type = ' + stTranType);    		        	
                		        	}
                		        }	        	
                	        }	 
                	        else
                	        {
                	        	...
                	        	for (var k = 0; k < arrSOInv.length; k++)
            		        	{
                	        		...
                	        		nlapiSubmitField(stTranType, stTranId, 'custbody_notification_processed', 'T');
            		        		nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Successfully updated Notification Process to true. ID = ' + stTranId
            		        				+ '\n <br /> Type = ' + stTranType);
            		        	}
                	        }
            	        }
    	        ...
    	        {
    	        	nlapiLogExecution('DEBUG', LOGGER_TITLE, 'Customer Billing Schedule is not found for the Transaction and Milestone. Marking of Notify Vendor for this record will be skipped.');
    	        }
            }
    	}
    	else
...
/**
 * Search if there is an existing Customer Billing Schedule using the Transaction and Milestone
 * @param stTranId
 * @param stTranType
 * @param stMilestone
 * @returns {String}
 */
function searchCustomerBillingSchedule(stTranId, stTranType, stMilestone)
{		
	var stCustomerBillingSchedule = '';
	var arrFilters = [new nlobjSearchFilter('custrecord_cbs_line_milestone', null, 'anyof', stMilestone)];
	
	if (stTranType == 'CustInvc')
	{
		arrFilters.push(new nlobjSearchFilter('custrecord_cbs_invoice', null, 'anyof', stTranId));        
	}
	else if (stTranType == 'SalesOrd')
	{
		arrFilters.push(new nlobjSearchFilter('custrecord_cbs_sales_order', null, 'anyof', stSalesOrder));
	}
 	 	
 	var arrCustomerBillingSchedule = nlapiSearchRecord('customrecord_customer_billing_schedules', null, arrFilters);
 	if (arrCustomerBillingSchedule != null)
 	{
 		stCustomerBillingSchedule = arrCustomerBillingSchedule[0].getId();
 	}
 	
 	return stCustomerBillingSchedule;
}
